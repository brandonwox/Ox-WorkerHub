...
Priority end-date picker no longer cut off (work request priority → Set dates): the End field's calendar popover now aligns its RIGHT edge to the field's right edge (the shared DateField gained an `align` prop passed through to DropdownPortal, which already supported 'right'); the Start field keeps left alignment. The 252px-wide picker previously opened left-edge-aligned from a field at the form's right edge and ran off the screen.

Pass 18 — Notifications overhaul + "Changes Saved" cleanup (no DB migration needed — the notifications.type column is unconstrained text). Scope per developer decisions: yes to everything from the audit, EXCEPT installers get no new notifications (only their existing today's-schedule-change ping + the device-local save-failed warning), and push-when-app-closed is deferred to the EAS/TestFlight move.
- 8 NEW NOTIFICATION TYPES: status_reported (field status set, with note → schedulers + the job's field supers, minus reporter — only on actual change); work_request_created (field-super-created card → schedulers; skipped for "Now" cards which already ping); issue_raised (fires when the issue's description is FIRST typed, not on the blank insert → schedulers + job's supers, minus raiser); issue_resolved (→ job's supers, minus resolver; reopening is silent); work_request_scheduled (scheduler schedules/moves a card, any date → job's supers); job_assigned (newly-added super via the assignment pickers, minus self-assigns → them); job_needs_qbt (job created with no QBT jobcode → finance managers); qbt_push_result (weekly QBT push ran → operator + finance managers — NOTE: fires from the client markTimesheetsSent action; once the sweep is fully server-side, the server job must insert these rows itself).
- FOREMAN STATUS REMINDER REVISED: a work request on the calendar for ANY past day (any hour), or for today once 4:30 PM local passes (was 3:30, today+yesterday only), with status still Undefined pings the crew's foreman — daily until set. The installer catch-up popup mirrors the same rule.
- MOBILE BELL: phones now have notification history — a floating bell (bottom-right, above the tab bar, unread badge) opens a full-screen sheet. Previously mobile had only the 5-second toasts.
- PANEL (shared web dropdown + mobile sheet, new components/notifications/NotificationList): All/Unread filter chips, Today/Earlier grouping, Mark all read + CLEAR ALL (bulk delete), per-type icons, dismiss X always visible on touch (hover-only on web).
- DEEP LINKS EVERYWHERE: clicking any notification row OR toast opens its target — work request notifications open the universal work request page (the scheduler's "Now" ping keeps its calendar quick-view jump), job notifications open the job (mobile job page on native; on web the role's jobs page now accepts an openJob param that pops the dashboard sidebar — field super, scheduler, operator).
- TOASTS: per-type icon (was always alert-circle), tap-to-open (was acknowledge-only), and native devices vibrate (RN Vibration; sound stays web-only).
- PER-TYPE MUTES: a Notifications card in Settings (both platforms) lists the types the viewer's role can receive; muting is device-local (AsyncStorage), muted types still land on the bell but never toast/ping/vibrate. save_failed is deliberately unmutable.
- RETENTION: after the login fetch, the client best-effort deletes its own notification rows older than 30 days (table previously grew forever; fetch stays capped at 100).
- "CHANGES SAVED" CLEANUP: the pill now fires only for deliberate discrete saves. Silenced: calendar drag assignment/unassignment, day reordering (one drag fired a pill per re-numbered card), task checkbox ticks, inline typing commits (photo notes, issue descriptions, installer field notes), photo SGD toggles, document retags, background writes (priority escalations, reminder stamps, the QBT sweep flag). Mechanism: outbox ops carry a persisted `silent` flag (write()'s new opts) alongside the existing silent-kind set.

Pass 17 — Fonts: Poppins app-wide + Quantico wordmark — BUILT, THEN FULLY REVERTED same day (developer preferred the old fonts on sight; no DB migration was involved):
- WHAT IT WAS: theme.ts's `fonts` swapped Inter → Poppins (400/500/600/700), plus a new `fonts.brand` = Quantico_700Bold used only by the two "WorkerHub" wordmarks (web sidebar brand + sign-in page).
- THE REVERT: everything is back to Inter via @expo-google-fonts/inter (regular: Inter_400Regular, medium: Inter_500Medium, semiBold: Inter_600SemiBold, bold: Inter_700Bold), wordmarks back on fonts.bold, `fonts.brand` removed, Poppins/Quantico packages uninstalled. Inter is the app's font by CHOICE now, not by default — don't re-propose Poppins.
- LESSON KEPT: the root layout now loads Inter via per-weight DEEP imports (@expo-google-fonts/inter/400Regular etc., useFonts from expo-font) instead of the package root — a package root require()s EVERY variant's ttf, which broke Metro bundling for Poppins mid-pass and silently bloats web exports with unused weights. Never import an @expo-google-fonts package root.

Pass 16 — Job details editing: PO + assigned Field Supers, and the flashing warning reword (REQUIRES: apply the new field-super-manage-assignments Supabase migration — it widens the job_field_supers write policy from operator + scheduler to operator + scheduler + field_super, replacing the self-assign-only policies):
- PO EDITABLE ON JOB DETAILS: the web job dashboard sidebar's Edit mode now has an inline PO editor in the header (where the PO displays; commit on blur, blank clears it), and the phone Field Super jobs list's inline editor gained a PO field. The Operator's Edit-job modal already had one.
- ASSIGNED FIELD SUPERS EDITABLE ON JOB DETAILS: the sidebar's Edit mode and the phone inline editor both gained the Field Supers chip picker (same picker as the Operator's modal — tap to assign/unassign, any number of supers). Shown on parent jobs only; sub-jobs inherit their parent's supers via the existing store mirror + DB trigger. "Assign myself" stays for supers browsing a job they're not on.
- FLASHING WARNING REWORD: the job details warning now reads "No Window Opening Flashing Material set — Installers need this information." (was "…work requests can't be created for this job until it is."). Wording only — the create-time block and its own warning inside the work request form are unchanged.
...
- SWIPE DOWN TO CLOSE (open work request, native): dragging down while the content is scrolled to the top pulls the sheet with the finger; past ~140px (or a quick flick) it slides off and closes, otherwise it springs back. The x button stays.
- KEYBOARD "DONE" BAR (iOS): a Done bar renders above the keyboard for the multiline inputs where there was no way to dismiss it — field notes, issue descriptions, camera photo notes, status-change notes, text documents. New shared component KeyboardDoneBar (mounted at the root; inputs opt in by id); Android/web untouched, they have their own affordances.
- FIELD SUPER ON THE WORK REQUEST: the open work request now shows the job's responsible field super (first-assigned) with their phone number on the same line — visible to installers; tapping it calls (native). And field supers now need a phone to operate: the Add/Edit worker forms gained a phone field (they previously never collected one at all — the invite function and workers.phone column already supported it) which is REQUIRED when the role is Field Super.

Pass 13 — Sub-job auto-PO + small UI fixes (no DB migration needed):
- SUB-JOB AUTO-PO: in the New Sub-Job popup, typing the name auto-fills the PO as parent PO + the typed piece (name "126" under parent PO "4500" → PO "4500126"; the type prefix never enters the PO). The PO field stays visible and editable — typing into it directly takes over and stops the auto-fill for that popup session.
- "+ ISSUE" ON JOB DETAILS: the Issues section title (web job dashboard sidebar AND mobile job details page) has a "+ Issue" button that raises a job-level issue attached to no work request. The new card starts expanded for its description; open-issue cards on the job pages are now creator-editable (they were read-only, which would have made the new issue untypable).
- NOTIFICATION DISMISS FIX: the x no longer vanishes as the pointer reaches it. Root cause: the button was UNMOUNTED when the row's hover ended, and on web the row's hover-out fires as the pointer crosses onto the nested button. It now stays mounted (hidden via opacity), re-asserts the row's hover itself, and taps with no hover forward to the row's open action.
- SETTINGS OUT OF THE WEB SIDEBAR: no role's left sidebar lists Settings anymore — the top-right profile chip opens it (already did). The top bar still titles the page "Settings" there (previously the fallback would have mistitled it once the nav entry was gone).
- POPUP SHADOWS ACTUALLY RENDER ON WEB: popups already declared modalShadow, but it used native shadow* props, which don't render on web — so popups (e.g. the scheduler's work request popup) showed no shadow at all. modalShadow now emits a boxShadow string on web ("0 6px 20px", subtle per theme); native keeps the original props untouched (web-only per the developer).
- "SERVICE" SCOPE RETIRED: removed from the selectable scope list everywhere (job/sub-job/work request forms). Rows already carrying it still read and display; the value stays in the type for that reason.

Pass 12 — Job editing: Builder field + Sub-Jobs overhaul (REQUIRES: apply the new job-builder-sub-job-type Supabase migration — it adds jobs.builder + jobs.sub_job_type and pins both in the role update guards). All on the web job dashboard sidebar (the mobile job details page has no edit mode):
- BUILDER FIELD: the edit-mode additional-info block gains a "Builder" input — a searchable dropdown of every builder ever applied to a job; typing an unmatched name and pressing Enter creates it. Editable by the roles with the full edit pencil (Operator / Field Supers); installers, schedulers, and finance managers are guard-blocked.
- 3-DOTS MENU REMOVED: the job details top-right now has only the Edit pencil. The menu's contents moved into the edit-mode block: the "This job has Sub-Jobs" toggle AND the Delete row (delete had no stated destination, so it rode along — it keeps its confirmation popup, as does deactivating sub-jobs). Roles that only had the 3-dots (e.g. schedulers on non-windows jobs) now get the pencil so nothing became unreachable.
- SUB-JOB TYPE: enabling "This job has Sub-Jobs" now requires choosing what the sub-jobs are called — "Lots", "Phases", "Bldgs", or a custom typed term (the choice is what turns the section on; stored on jobs.sub_job_type). The type stays visible/changeable in edit mode while the toggle is on.
- SUB-JOB NAMING: the create-sub-job form shows the parent name chip, then the singular type chip ("Lot"), then the name input — typing "159" stores the name "Lot 159". The placeholder becomes type-aware ("Which lot is this?"); parents with no saved type (enabled before this pass) keep the old free-text naming and placeholder until a type is picked in edit mode.

Pass 11 — PO display + work request card presentation (no DB migration needed). The connected "show the PO, not the job name / not a 'PO' label" edits done as one batch:
- INSTALLER DAILY SCHEDULE CARDS (mobile home agenda + desktop installer-schedule): each card now shows the linked job's PO directly above the work request title, the address below it (address alone — the job name moved out in favor of the PO), and the flashing-material line is replaced by the work request's notes. Every line still truncates to one line.
- LIGHT MODE SCHEDULE CARDS: the daily-schedule card background is now slightly darker (#F6F7F9) than the page behind it in light mode (it was white-on-near-white); open work requests and dark mode are untouched.
- OPEN WORK REQUEST (mobile): the shadow above the top of the sheet is removed (no borders remained). Also, the parent-job link above the title now shows the PO (falls back to the job name for legacy jobs with no PO).
- "PO" PREFIX DROPPED: job details pages (mobile job-site page + web job dashboard sidebar, incl. their Sub-Jobs lists) and every column list of jobs (scheduler/field-super/operator web + mobile, finance, and the Pics job list) now show the PO by itself with no "PO " label. Search placeholders and form field labels keep the word.
- FIELD-SUPER-WORK-REQUESTS PAGE: the text under each work request's name is now just the PO (a sub-job shows its OWN PO; legacy no-PO jobs fall back to the job name) — on web (opt-in prop on the shared screen, so the scheduler's page keeps job names) and on the mobile list. Inside the open work request the label above the name is also just the PO (web quick view + the mobile work request page). New shared helper: workRequestPoLabel in utils/workRequestJobs.ts.
